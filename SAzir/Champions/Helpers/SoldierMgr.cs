...
namespace SAutoCarry.Champions.Helpers
{
    public static class SoldierMgr
    {
        ...
        {
            return (float)ObjectManager.Player.GetSpellDamage(target, SpellSlot.W) * ActiveSoldiers.Count(p => p.Position.Distance(target.Position) < SoldierAttackRange);
        }
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe)
            {
                if (args.SData.Name.ToLower() == "azirbasicattacksoldier")
                {
                    ...
                    Utility.DelayAction.Add((int)(ObjectManager.Player.AttackCastDelay * 1000), () => SoldierAttacking = false);
                }
